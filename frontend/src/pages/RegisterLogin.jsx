...
import { loginRecruiter } from "../app/auth/authThunks";

const RegisterLogin = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { loading } = useSelector((state) => state.auth); // ✅ make sure this matches your slice name
...
const loginSubmit = async (e) => {
  ...
  try {
    const res = await dispatch(loginRecruiter(formData)).unwrap();

    // ✅ No need to use `res.data` — you're already returning res.data from the thunk
    // const token = res.token;
    const recruiter = res.user;
